...
class DataFrameImputer(BaseEstimator,TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        for col in X.columns:
            if X[col].dtype=='O':
                self.impute_dict[col]=X[col].mode()
            else:
...
class Conver_Datetime(BaseEstimator,TransformerMixin) :
...
class Date_Info_Extract(BaseEstimator,TransformerMixin):
    def __init__(self,DIE):              
         self.DIE = [D.lower() for D in DIE]
         self.feature_names=[]
         self.week_day = 7
         self.month = 21
         self.day = 31
    ...
    def fit(self,x,y=None):
        ...
        for col in x.columns:
           for temp in self.DIE :
               self.feature_names.extend([col+ '_' + temp ])
               for y in ['sin','cos']:
                    self.feature_names.extend([col+ '_' + temp + '_'+ y])
        return self
    ...
    def transform(self, X, y=None):
        ...
        for col in X.columns:
            if('weekday' in self.DIE):
                data[col + '_' + 'weekday'] = X[col].dt.weekday
                data[col + '_' + 'weekday' + '_sin'] = np.sin((2 * np.pi * X[col].dt.weekday)/self.week_day)
                data[col + '_' + 'weekday' + '_cos'] = np.cos((2 * np.pi * X[col].dt.weekday)/self.week_day)
            if('month' in self.DIE):
                data[col +'_'+ 'month'] = X[col].dt.month
                data[col + '_' + 'month' + '_sin'] = np.sin((2 * np.pi * X[col].dt.month)/self.month)
                data[col + '_' + 'month' + '_cos'] = np.cos((2 * np.pi * X[col].dt.month)/self.month)
            if('day' in self.DIE):
                data[col+'_' + 'day'] = X[col].dt.day
                data[col + '_' + 'day' + '_sin'] = np.sin((2 * np.pi * X[col].dt.day)/self.day)
                data[col + '_' + 'day' + '_cos'] = np.cos((2 * np.pi * X[col].dt.day)/self.day)
...
class Day_Diff(BaseEstimator,TransformerMixin):
    
    def __init__(self):

        self.feature_names=['Day Diff']
    ...
    def transform(self,X,y = None):
        ...
        Day_Dif = (X.iloc[:,1] - X.iloc[:,0]).dt.days
        return pd.DataFrame(data = {"Day Diff":Day_Dif})
